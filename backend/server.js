    ...
    transf.aggregate(pipeline3, (err, frankdata) => {
      transc.aggregate(pipeline1, (err, camsdata) => {
       transk.aggregate(pipeline2, (err, karvydata) => {
        if(karvydata != 0 || camsdata != 0 ||  frankdata!=0){
            resdata = {
                status: 200,
                message: 'Successfull',
                data: karvydata
...
app.post("/api/gettransactionuserwise", function (req, res) {
    ...
    var transk = mongoose.model('trans_karvy', transkarvy, 'trans_karvy');
    var transf = mongoose.model('trans_franklin', transfranklin, 'trans_franklin');
   if ( req.body.pan === "Please Provide" || req.body.pan === "") {
...
app.post("/api/gettaxsavinguserwise", function (req, res) {
    ...
    if ( req.body.pan === "Please Provide" || req.body.pan === "" || req.body.pan === "Not An Assessee") {
        ...
        const pipeline1 = [  ///trans_karvy
            { $match: { $and: [{ FUNDDESC: /Tax/ }, { INVNAME: req.body.name }, { TRDESC: { $not: /^Redemption.*/ } },{ TRDESC: { $not: /^Rejection.*/ } },{ TRDESC: { $not: /^Gross Dividend.*/ } },{ TRDESC: { $not: /^Switch Over Out.*/ } },  { TRDESC: { $not: /^Dividend Paid.*/ } }, { TRDESC: { $not: /^Switchout.*/ } }, { TRDESC: { $not: /^Transfer-Out.*/ } }, { TRDESC: { $not: /^Lateral Shift Out.*/ } },{ TD_TRDT: { $gte: new Date(moment(yer).format("YYYY-MM-DD")), $lt: new Date(moment(secyer).format("YYYY-MM-DD")) } } ] } },
            { $group: { _id: { INVNAME: "$INVNAME", PAN1: "$PAN1", FUNDDESC: "$FUNDDESC", TRDESC: "$TRDESC", TD_ACNO: "$TD_ACNO", TD_AMT: "$TD_AMT", TD_TRDT: "$TD_TRDT" } } },
...
app.post("/api/gettaxsaving", function (req, res) {
    ...
    const pipeline1 = [  ///trans_karvy
        { $match: { $and: [{ FUNDDESC: /Tax/ }, { TRDESC: { $not: /^Redemption.*/ } }, { TRDESC: { $not: /^Dividend.*/ } }, { TRDESC: { $not: /^Switchout.*/ } }, { TRDESC: { $not: /^Transfer-Out.*/ } }, { TRDESC: { $not: /^Lateral Shift Out.*/ } } ,{ TD_TRDT: { $gte: new Date(moment(yer).format("YYYY-MM-DD")), $lt: new Date(moment(secyer).format("YYYY-MM-DD")) } } ] } },
        { $group: { _id: { FUNDDESC: "$FUNDDESC", TRDESC: "$TRDESC", TD_ACNO: "$TD_ACNO", TD_AMT: "$TD_AMT", TD_TRDT: "$TD_TRDT" } } },
    ...
    const pipeline2 = [  ///trans_franklin
        { $match: { $and: [{ SCHEME_NA1: /Tax/ }, { TRXN_TYPE: { $not: /^Redemption.*/ } }, { TRXN_TYPE: { $not: /^Dividend.*/ } }, { TRXN_TYPE: { $not: /^Switchout.*/ } }, { TRXN_TYPE: { $not: /^Transfer-Out.*/ } }, { TRXN_TYPE: { $not: /^Lateral Shift Out.*/ } } ,{ TRXN_DATE: { $gte: new Date(moment(yer).format("YYYY-MM-DD")), $lt: new Date(moment(secyer).format("YYYY-MM-DD")) } } ] } },
        { $group: { _id: { SCHEME_NA1: "$SCHEME_NA1", TRXN_TYPE: "$TRXN_TYPE", FOLIO_NO: "$FOLIO_NO", AMOUNT: "$AMOUNT", TRXN_DATE: "$TRXN_DATE" } } },
...
//     var unit=0;var balance=0;var currentvalue=0;var amt=0;
//     const pipeline1 = [  //trans_cams
//         { $match: { FOLIO_NO: req.body.folio, SCHEME: req.body.scheme } },
//         { $group: { _id: { FOLIO_NO:"$FOLIO_NO",INV_NAME: "$INV_NAME",SCHEME:"$SCHEME",BANK_NAME: "$BANK_NAME", AC_NO: "$AC_NO",TRXN_TYPE_: "$TRXN_TYPE_",AMC_CODE: "$AMC_CODE", PRODCODE: "$PRODCODE", code:{ $ltrim: { input: "$PRODCODE", chars: "$AMC_CODE" } } } , UNITS: { $sum: "$UNITS" }, AMOUNT: { $sum: "$AMOUNT" }  } },
//         {
//             $lookup:
//             {
//                 from: "products",
...
//          { $lookup: { from: 'folio_cams', localField: '_id.FOLIO_NO', foreignField: 'FOLIOCHK', as: 'detail' } },
//          { $unwind: "$detail" },
//          { $project: { _id: 0, FOLIO:"$_id.FOLIO_NO",INVNAME: "$_id.INV_NAME",SCHEME:"$_id.SCHEME",NATURE: "$_id.TRXN_TYPE_",ISIN: "$products.ISIN",NOMINEE: "$detail.NOM_NAME", BANK_NAME: "$_id.BANK_NAME", AC_NO: "$_id.AC_NO",JTNAME2: "$detail.JNT_NAME2", JTNAME1: "$detail.JNT_NAME1", cnav: "$nav.NetAssetValue",  UNITS: { $sum: "$UNITS" }, AMOUNT: { $sum: "$AMOUNT" } } },
//     ]

//     const pipeline2 = [  //trans_karvy
//         { $match: { TD_ACNO: req.body.folio, FUNDDESC: req.body.scheme } },
//         { $group: { _id: {TD_ACNO:"$TD_ACNO", INVNAME: "$INVNAME",FUNDDESC:"$FUNDDESC",TD_TRTYPE: "$TD_TRTYPE", SCHEMEISIN: "$SCHEMEISIN" }, TD_UNITS: { $sum: "$TD_UNITS" } , TD_AMT: { $sum: "$TD_AMT" }} },
//         { $lookup: { from: 'cams_nav', localField: '_id.SCHEMEISIN', foreignField: 'ISINDivPayoutISINGrowth', as: 'nav' } },
//         { $unwind: "$nav" },
//         { $lookup: { from: 'folio_karvy', localField: '_id.TD_ACNO', foreignField: 'ACNO', as: 'detail' } },
//         { $unwind: "$detail" },
//         { $project: { _id: 0, FOLIO:"$_id.TD_ACNO",INVNAME: "$_id.INVNAME",SCHEME:"$_id.FUNDDESC",NATURE: "$_id.TD_TRTYPE",ISIN: "$_id.SCHEMEISIN" ,NOMINEE: "$detail.NOMINEE",  BANK_NAME: "$detail.BNAME" ,AC_NO: "$detail.BNKACNO", JTNAME2: "$detail.JTNAME2", JTNAME1: "$detail.JTNAME1",cnav: "$nav.NetAssetValue", UNITS: { $sum: "$TD_UNITS" }, AMOUNT: { $sum: "$TD_AMT" } } } ,
//     ]

//     const pipeline3 = [  //trans_franklin
//         { $match: { FOLIO_NO: req.body.folio, SCHEME_NA1: req.body.scheme } },
//         { $group: { _id: { FOLIO_NO:"$FOLIO_NO", INVESTOR_2: "$INVESTOR_2",SCHEME_NA1:"$SCHEME_NA1",TRXN_TYPE: "$TRXN_TYPE", ISIN: "$ISIN", NOMINEE1: "$NOMINEE1", PBANK_NAME: "$PBANK_NAME", PERSONAL23: "$PERSONAL23", JOINT_NAM2: "$JOINT_NAM2", JOINT_NAM1: "$JOINT_NAM1" }, UNITS: { $sum: "$UNITS" }, AMOUNT: { $sum: "$AMOUNT" } } },
//         { $lookup: { from: 'cams_nav', localField: '_id.ISIN', foreignField: 'ISINDivPayoutISINGrowth', as: 'nav' } },
//         { $unwind: "$nav" },
//         { $project: { _id: 0,FOLIO:"$_id.FOLIO_NO", INVNAME: "$_id.INVESTOR_2",SCHEME:"$_id.SCHEME_NA1", NATURE: "$_id.TRXN_TYPE",ISIN: "$_id.ISIN", NOMINEE: "$_id.NOMINEE1", BANK_NAME: "$_id.PBANK_NAME", AC_NO: "$_id.PERSONAL23", JTNAME2: "$_id.JOINT_NAM2", JTNAME1: "$_id.JOINT_NAM1", cnav: "$nav.NetAssetValue", UNITS: { $sum: "$UNITS" }, AMOUNT: { $sum: "$AMOUNT" } } },
//     ]
...
//                         for (var i = 0; i < datacon.length; i++) {
//                             if (datacon[i]['NATURE'] === "RED" || datacon[i]['NATURE'] ==='FUL' ||
//                             datacon[i]['NATURE'] ==='LTOP' || datacon[i]['NATURE'] ==='LTOF' || 
//                             datacon[i]['NATURE'] ==='STPO' || datacon[i]['NATURE'] ==='IPOR') {
//                                 unit = "-"+datacon[i].UNITS;
//                               }else{
//                                 unit = datacon[i].UNITS;
//                               }
//                             balance = parseFloat(unit)+parseFloat(balance) 
//                             datacon[i].UNITS = balance;
//                             datacon[i].AMOUNT = parseFloat(datacon[i].cnav)*parseFloat(balance) ;
//                             datacon[i].BANK_NAME =  datacon[0].BANK_NAME;
//                             datacon[i].AC_NO =  datacon[0].AC_NO
//                             datacon[i].NOMINEE =  datacon[0].NOMINEE
//                             datacon[i].JTNAME2 =  datacon[0].JTNAME2
//                             datacon[i].JTNAME1 =  datacon[0].JTNAME1
//                         }
              ...
              transf.aggregate(pipeline3, (err, frankdata) => {
                    ...
                    var datacon = frankdata.concat(karvydata.concat(camsdata));
                        ...
                        for (var i = 0; i < datacon.length; i++) {
                            ...
                            datacon[i].AMOUNT = parseFloat(datacon[i].cnav)*parseFloat(balance) ;
                            datacon[i].BANK_NAME =  datacon[0].BANK_NAME;
                            datacon[i].INVNAME =  datacon[0].INVNAME;
                            datacon[i].AC_NO =  datacon[0].AC_NO
                            datacon[i].NOMINEE =  datacon[0].NOMINEE
                            datacon[i].JTNAME2 =  datacon[0].JTNAME2
                            datacon[i].JTNAME1 =  datacon[0].JTNAME1
...
app.post("/api/getschemedetail", function (req, res) {
    ...
    const pipeline1 = [  //trans_karvy    "TD_TRTYPE":{$not: /^SINR.*/}
        { $match: { FUNDDESC: req.body.scheme, PAN1: req.body.pan ,TD_ACNO:req.body.folio } },
        { $group: { _id: { PAN1: "$PAN1",FUNDDESC:"$FUNDDESC", TD_NAV: "$TD_NAV", TD_TRTYPE: "$TD_TRTYPE", TD_TRDT: "$TD_TRDT", TRDESC: "$TRDESC", INVNAME: "$INVNAME", SCHEMEISIN: "$SCHEMEISIN", cnav: "$nav.NetAssetValue" }, TD_UNITS: { $sum: "$TD_UNITS" }, TD_AMT: { $sum: "$TD_AMT" } } },
        { $lookup: { from: 'cams_nav', localField: '_id.SCHEMEISIN', foreignField: 'ISINDivPayoutISINGrowth', as: 'nav' } },
        //   { $unwind: "$nav"},
        { $project: { _id: 0, PAN: "$_id.PAN1", FUNDDESC:"$_id.FUNDDESC",TD_NAV: "$_id.TD_NAV", NATURE: "$_id.TD_TRTYPE", TD_TRDT: { $dateToString: { format: "%d-%m-%Y", date: "$_id.TD_TRDT" } }, TRDESC: "$_id.TRDESC", INVNAME: "$_id.INVNAME", SCHEMEISIN: "$_id.SCHEMEISIN", cnav: "$nav.NetAssetValue", UNITS: { $sum: "$TD_UNITS" }, AMOUNT: { $sum: "$TD_AMT" } } },
        //  {$sort : {TD_TRDT : -1}}
    ]
    // const pipeline1=[  //trans_karvy   
    ...
    transk.aggregate(pipeline1, (err, newdata1) => {
        ...
        // for (var i = 0; i < datacon.length; i++) {
        //     if (datacon[i]['NATURE'] === "LTOP" || datacon[i]['NATURE'] === "LTOF") {
        //         datacon[i]['NATURE'] = "Switch Out";
        //     }
        //     if ((Math.sign(datacon[i]['AMOUNT']) === -1)) {
...
app.post("/api/getschemeportfoliodetail", function (req, res) { 
    const pipeline1 = [  //trans_karvy    "TD_TRTYPE":{$not: /^SINR.*/}
        { $match: { "FUNDDESC": req.body.scheme, "PAN1": req.body.pan, "TD_ACNO":req.body.folio ,"STATUS":"INDIVIDUAL"} },
        { $group: { _id: {  TD_ACNO: "$TD_ACNO", FUNDDESC: "$FUNDDESC", TD_TRTYPE: "$TD_TRTYPE", TD_TRDT: "$TD_TRDT", SCHEMEISIN: "$SCHEMEISIN", cnav: "$nav.NetAssetValue",navdate:"$nav.Date" }, TD_UNITS: { $sum: "$TD_UNITS" }, TD_AMT: { $sum: "$TD_AMT" } } },
        { $lookup: { from: 'cams_nav', localField: '_id.SCHEMEISIN', foreignField: 'ISINDivPayoutISINGrowth', as: 'nav' } },
        //  { $unwind: "$nav"},
        { $project: { _id: 0,  FOLIO: "$_id.TD_ACNO", SCHEME: "$_id.FUNDDESC", NATURE: "$_id.TD_TRTYPE", TD_TRDT: { $dateToString: { format: "%d-%m-%Y", date: "$_id.TD_TRDT" } },ISIN: "$_id.SCHEMEISIN",   cnav: "$nav.NetAssetValue",navdate: "$nav.Date" , UNITS: { $sum: "$TD_UNITS" }, AMOUNT: { $sum: "$TD_AMT" } } },
        {$sort : {SCHEME : 1}}
    ]
    transk.aggregate(pipeline1, (err, newdata1) => {
